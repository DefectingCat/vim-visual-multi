...
local function record(name, priority, actual, expected)
  ...
  if passed then
    ...
    print("[INFO][" .. priority .. "] " .. name .. ": actual=" .. tostring(actual) .. " (expected: " .. tostring(expected) .. ")")
  end
...
function M.run_all()
  ...
  for name, r in pairs(M.results) do
    if not r.passed then
      if r.priority == "p1" then p1_fails = p1_fails + 1
      elseif r.priority == "p2" then p2_fails = p2_fails + 1
      end
    end
  end
