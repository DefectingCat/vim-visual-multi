...
function M.regions_contents()
  ...
  for _, r in ipairs(regions) do
    local info = string.format('Region %d: line=%d-%d, col=%d-%d, text="%s"',
      r.index, r.l, r.L, r.a, r.b, r.txt or '')
    print(info)
  end
end
...
function M.regions_to_buffer()
  local regions = R_fn()
  if X_fn() == 0 or #regions == 0 then return end
...
function M.filter_regions(type, exp, prompt)
  ...
  if prompt then
    ...
    vim.fn.execute('cnoremap <buffer><nowait><silent><expr> <C-x> <SID>filter_regions(getcmdline())')
