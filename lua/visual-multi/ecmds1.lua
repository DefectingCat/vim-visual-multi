...
function M.init()
  ...
  min_fn = function(n) return X_fn() and #R_fn() >= n end
...
function Edit.paste(before, vim_reg, reselect, register, ...)
  ...
  local use_vim_reg = vim_reg or not vim.g.Vm.registers[register] or
                      vim.tbl_isempty(vim.g.Vm.registers[register])
...
function Edit.replace_chars()
  ...
  if X_fn() then
    ...
    if char:upper() == '<ESC>' then return end
