...
local function s_init(whole, type, extend_mode)
  ...
  if vim.g.Vm.buffer then
    if v.using_regex then M.regex_reset() end
    v.whole_word = whole
...
local function skip_shorter_lines()
  ...
  if vim.g.VM_skip_shorter_lines == 1 then
    ...
    local endline = vim.g.VM_skip_empty_lines == 1
        and vim.fn.virtcol('$')
...
function M.find_by_regex(mode)
  ...
  vim.fn.execute('cnoremap <silent><nowait><buffer> <esc><esc> <C-u><C-r>=b:VM_Selection.Vars.regex_backup<cr><esc>:call vm#commands#regex_abort()<cr>')
  vim.fn.execute('cnoremap <silent><nowait><buffer> <esc> <C-u><C-r>=b:VM_Selection.Vars.regex_backup<cr><esc>:call vm#commands#regex_abort()<cr>')
...
local function is_r()
  ...
  return vim.g.Vm.buffer and Global.region_at_pos() and not vim.tbl_isempty(Global.region_at_pos())
end
...
function M.find_next(skip_flag, nav)
  ...
  if (nav or skip_flag) and Funcs.no_regions() then return end
  ...
  if not Search.validate() and not nav then return end
...
function M.skip(just_remove)
  ...
  if Funcs.no_regions() then return end
...
function M.invert_direction(...)
  ...
  if Funcs.no_regions() or v.auto then return end
