...
function M.update_regions()
  ...
  if X_fn() == 1 then
    for _, r in ipairs(R_fn()) do
      if r.update_region then r:update_region() end
    end
  else
    for _, r in ipairs(R_fn()) do
      if r.update_cursor then r:update_cursor() end
    end
  end
...
function M.reset_vars()
  if v.eco == 0 and v.auto == 0 then return end
...
function M.merge_regions()
  ...
  if X_fn() == 0 then return M.merge_cursors() end
...
function M.merge_maps(map)
  ...
  if vim.tbl_isempty(v_bytes) then return {} end
